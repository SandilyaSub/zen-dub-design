#!/usr/bin/env python3
"""
Test script for Llama translation module via Together API.

This script tests the Llama translation module by:
...
Usage:
    python3 tests/test_llama_translation.py --input_file /path/to/diarization.json --source_language hindi --target_language english --api_key YOUR_TOGETHER_API_KEY
"""
...
from modules.llama_translation import (
    llama_translate_diarized_content,
    llama_back_translate,
    llama_translate_diarized_content_context_aware
)
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Test Llama translation module via Together API')
    ...
    # Set API key in environment for modules that might use it
    ...
    try:
        ...
        if args.context_aware:
            translated_data = llama_translate_diarized_content_context_aware(
                diarization_data,
    ...
    translated_file = output_dir / f"{base_filename}_llama_translated_{timestamp}.json"
    ...
    try:
        ...
        if args.context_aware:
            back_translated_data_raw = llama_translate_diarized_content_context_aware(
            ...
            back_translated_data_raw = llama_translate_diarized_content(
                back_translation_input,
                args.source_language,
                args.target_language,
                api_key=args.api_key
    ...
    back_translated_file = output_dir / f"{base_filename}_llama_back_translated_{timestamp}.json"
    ...
    # Save metrics
    metrics_file = output_dir / f"{base_filename}_llama_metrics_{timestamp}.json"
