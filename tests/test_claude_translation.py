#!/usr/bin/env python3
"""
Test script for Claude translation module via Vertex AI.

This script tests the Claude translation module by:
...
Usage:
    python3 tests/test_claude_translation.py --input_file /path/to/diarization.json --source_language hindi --target_language english --project_id your_gcp_project_id --region us-east5
"""
...
from modules.claude_translation import (
    claude_translate_diarized_content,
    claude_back_translate,
    claude_translate_diarized_content_context_aware
)
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Test Claude translation module via Vertex AI')
    ...
    parser.add_argument('--project_id', help='Google Cloud Project ID (optional, falls back to env var)')
    parser.add_argument('--region', default='us-east5', help='Google Cloud region (default: us-east5)')
    ...
    if args.project_id:
        logger.info("Using project ID from command line arguments")
        os.environ['VERTEX_PROJECT_ID'] = args.project_id
    
    # Set region in environment if provided
    if args.region:
        logger.info(f"Using region from command line arguments: {args.region}")
        os.environ['VERTEX_REGION'] = args.region
    ...
    try:
        ...
        if args.context_aware:
            translated_data = claude_translate_diarized_content_context_aware(
                diarization_data,
            ...
            translated_data = claude_translate_diarized_content(
                diarization_data,
                args.target_language,
                args.source_language,
                project_id=args.project_id,
    ...
    translated_file = output_dir / f"{base_filename}_claude_translated_{timestamp}.json"
    ...
    try:
        ...
        if args.context_aware:
            back_translated_data_raw = claude_translate_diarized_content_context_aware(
            ...
            back_translated_data_raw = claude_translate_diarized_content(
                back_translation_input,
                args.source_language,
                args.target_language,
                project_id=args.project_id,
    ...
    back_translated_file = output_dir / f"{base_filename}_claude_back_translated_{timestamp}.json"
    ...
    # Save metrics
    metrics_file = output_dir / f"{base_filename}_claude_metrics_{timestamp}.json"
