"""
Llama Translation module using Together API for diarized translation.
"""
import os
import json
import logging
...
log_file = f"logs/llama_translation_{current_date}.log"
...
logger.info(f"TOGETHER_API_KEY found: {'Yes' if TOGETHER_API_KEY else 'No'}")
...
def translate_with_validation(client, input_json: Dict, system_prompt: str, max_retries: int = 2) -> Dict:
    """
    Translate with validation and retry logic using Llama via Together API
    
    Args:
        client: The Together API client
    ...
    while attempts <= max_retries:
        try:
            ...
            logger.info(f"Sending translation request to Together API (attempt {attempts+1}/{max_retries+1})")
            ...
            logger.info(f"Received response from Together API: {response_text[:100]}...")
...
def translate_text(text: str, source_lang: str, target_lang: str, mode: str = "modern-colloquial", api_key: str = None) -> str:
    """
    Translate text using Llama via Together API.
    ...
    Args:
        ...
        api_key: Together API key (optional, falls back to env var)
        
    Returns:
        Translated text
    """
    # Get API key
    together_api_key = api_key or TOGETHER_API_KEY
    if not together_api_key:
        error_msg = "No Together API key provided or found in environment variables"
...
"""
        ...
        # Call Together API
        logger.info("Sending translation request to Together API")
        response = client.chat.completions.create(
            model=DEFAULT_MODEL,
            messages=[
...
def llama_translate_diarized_content(diarization_data: Dict, target_language: str, source_language: str = "auto", api_key: str = None) -> Dict:
    """
    Translate diarized content using Llama via Together API.
    ...
    """
    # Debug info
    print(f"==== LLAMA TRANSLATION DEBUG ==== Starting llama_translate_diarized_content for target_language={target_language}, source_language={source_language}")
    
    # Get API key
    together_api_key = api_key or TOGETHER_API_KEY
    if together_api_key:
        print(f"==== LLAMA TRANSLATION DEBUG ==== TOGETHER_API_KEY found with length: {len(together_api_key)}")
    else:
        error_msg = "No Together API key provided or found in environment variables"
        logger.error(error_msg)
    ...
    print(f"==== LLAMA TRANSLATION DEBUG ==== Found {len(segments)} segments to translate")
    
    # Translate each segment
    for i, segment in enumerate(segments):
        if "text" in segment and segment["text"]:
            print(f"==== LLAMA TRANSLATION DEBUG ==== Translating segment {i+1}/{len(segments)}")
            ...
            if translated_text.startswith("Error:"):
                print(f"==== LLAMA TRANSLATION DEBUG ==== Error translating segment {i+1}: {translated_text}")
                continue
                
            segment["text"] = translated_text
            segment["original_text"] = original_text
            print(f"==== LLAMA TRANSLATION DEBUG ==== Segment {i+1} translated successfully")
    
    # Add translation info
    result["translation_info"] = {
        "source_language": source_language,
        "target_language": target_language,
        "translator": "llama_together",
        "translation_mode": "modern-colloquial"
    }
    
    print(f"==== LLAMA TRANSLATION DEBUG ==== Translation complete for all segments")
    return result

def llama_back_translate(translated_data: Dict, original_language: str, api_key: str = None) -> Dict:
    """
    Back-translate content using Llama via Together API.
    ...
    return llama_translate_diarized_content(
        translated_data,
        original_language,
        target_language,
        api_key=api_key
...
def llama_translate_diarized_content_context_aware(diarization_data, target_language, source_language="auto", api_key=None):
    """
    Translate diarized content using Llama with context awareness.
